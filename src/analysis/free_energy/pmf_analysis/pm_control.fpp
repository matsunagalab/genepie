...
contains
  ...
  !
  !  Subroutine    usage
  !> @brief        show usage of pmf_analysis
  !! @authors      NT
  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./pmf_analysis INP'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./pmf_analysis -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in pmf_analysis'
        write(MsgOut,'(A)') ' '
         
        call show_ctrl_input ('CV,WEIGHT,DIST')
  ...
  !> @brief        read control parameters from an in-memory string (library mode)
  !! @authors      NT
  !! @param[in]    ctrl_text : control file content as a C character buffer
  !! @param[in]    ctrl_len  : number of valid characters in ctrl_text
  !! @param[inout] ctrl_data : information of control parameters
  !
  !======1=========2=========3=========4=========5=========6=========7=========8
