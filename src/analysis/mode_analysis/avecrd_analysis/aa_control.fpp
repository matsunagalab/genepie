  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./avecrd_analysis INP'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./avecrd_analysis -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in avecrd_analysis'
        write(MsgOut,'(A)') ' '

        call show_ctrl_input ('psf,ref,prmtop,ambcrd,grotop,grocrd')
        call show_ctrl_output('pdb,rms,pdb_ave,pdb_aft')
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                 AVECRD_ANALYSIS                  *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*  Calculate average structure of trajectory files *'
      write(MsgOut,'(A)')'*                                                  *'
