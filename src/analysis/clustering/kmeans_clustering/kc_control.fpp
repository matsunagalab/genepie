  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./kmeans_clustering INP'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./kmeans_clustering -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in kmeans_clustering'
        write(MsgOut,'(A)') '# pdbfile and trjfile in [OUTPUT] can be optionally specified'
        write(MsgOut,'(A)') ' '

        call show_ctrl_input ('psf,ref,prmtop,ambcrd,grotop,grocrd,index')
        call show_ctrl_output('index,PDB,TRJ')
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                  KMEANS_ANALYSIS                 *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*       k-means clustering for MD trajectory       *'
      write(MsgOut,'(A)')'*                                                  *'
