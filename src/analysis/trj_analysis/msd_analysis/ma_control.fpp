...
module ma_control_mod
  ...
  type, public :: s_ctrl_data
    ...
    ! data for section molecule_selection
    type(s_molsel_info)      :: molsel_info
...
contains
  ...
  !
  !  Subroutine    usage
  !> @brief        show usage of msd_analysis
  !! @authors      DS
  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '# This program calculates mean square displacement.'
        write(MsgOut,'(A)') ''
        write(MsgOut,'(A)') '# normal usage'
        write(MsgOut,'(A)') '  % ./msd_analysis INP'
        write(MsgOut,'(A)') ''
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./msd_analysis -h ctrl'
        write(MsgOut,'(A)') ''

      else

        write(MsgOut,'(A)') '# control parameters in msd_analysis'
        write(MsgOut,'(A)') ''

        call show_ctrl_input ('pdb,psf,prmtop,ambcrd')
        call show_ctrl_output('msd')
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                  MSD_ANALYSIS                    *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                  MSD analyzer                    *'
      write(MsgOut,'(A)')'*                                                  *'
  ...
  subroutine control(filename, ctrl_data)
    ...
    ! read molecule selection section
    !
    call read_ctrl_molselect(handle, ctrl_data%molsel_info)

    ! read option section
    !
    call read_ctrl_option(handle, ctrl_data%opt_info, ctrl_data%molsel_info)
