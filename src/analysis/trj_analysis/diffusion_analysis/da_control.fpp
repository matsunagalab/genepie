  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') 'This program calculates translational diffusion coefficient.'
        write(MsgOut,'(A)') ''
        write(MsgOut,'(A)') '# normal usage'
        write(MsgOut,'(A)') '  % ./diffusion_analysis INP'
        write(MsgOut,'(A)') ''
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./diffusion_analysis -h ctrl'
        write(MsgOut,'(A)') ''

      else

        call show_ctrl_input ('msd')
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                DIFFUSION_ANALYSIS                *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                DIFFUSION analyzer                *'
      write(MsgOut,'(A)')'*                                                  *'
