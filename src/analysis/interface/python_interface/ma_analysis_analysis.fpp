...
contains
  ...
  subroutine analyze(molecule, trajes_c, ana_period, output, option, &
                     msd2, num_analysis_mols, num_delta)
...
#   endif /*_OPENMP || OMP*/
    ...
    real(wp), pointer,       intent(out)   :: msd2(:,:)
    ...
    allocate( msd2(size(option%analysis_mols), option%delta) )
...
#   endif /*_OPENMP || OMP*/
    ...
    write(MsgOut, '(A, i0)') "trajes_c%nframe=", trajes_c%nframe
    do istep_local = 1, trajes_c%nframe
      ...
      !
      call get_frame(trajes_c, istep_local, trajectory)

      ! respect analysis periods setting
      if (mod(istep_local-1, ana_period) /= 0) cycle
