...
module rmsd_impl_mod
  ...
  public  :: analyze_zerocopy_with_fitting
...
!      call error_msg('Analyze> mass weighted is not allowed')
  ...
  !======1=========2=========3=========4=========5=========6=========7=========8
  !
  !  Subroutine    analyze_zerocopy_with_fitting
  !> @brief        RMSD analysis with fitting (zerocopy version)
  !! @authors      Claude Code
  !! @param[in]    mass           : mass array pointer (view of Python NumPy)
  !! @param[in]    ref_coord      : reference coordinates (3, n_atoms)
  ...
  !! @param[in]    mass_weighted  : use mass weighting
  !! @param[out]   rmsd_results   : RMSD results (pointer to be allocated)
  !! @param[out]   nstru_out      : number of structures analyzed
  !! @note         This version performs structural fitting before RMSD calculation.
  !!               Original trajectory coordinates are NOT modified (uses work buffer).
  !
  !======1=========2=========3=========4=========5=========6=========7=========8

  subroutine analyze_zerocopy_with_fitting(mass, ref_coord, n_atoms, &
                              trajes_c, ana_period, &
    ...
    ! IMPORTANT: mass_fitting must be indexed by atom index (not loop counter)
    ! because fit_trrot accesses mass_w(aidx(i)) internally
    ...
    ! Output summary
    write(MsgOut,'(A)') ''
    write(MsgOut,'(A)') 'Analyze_zerocopy_with_fitting> RMSD analysis completed (zero-copy, with fitting)'
    write(MsgOut,'(A)') ''

    return

  end subroutine analyze_zerocopy_with_fitting
