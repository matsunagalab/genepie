!--------1---------2---------3---------4---------5---------6---------7---------8
!
!  Module   cc_control_mod
!> @brief   read parameters and data for MD trajectory converter
  ...
  subroutine usage(arg1)
    ...
    if (iargc() < 1      .or. &
      ...
      if (iargc() < 2 .or. arg2 /= 'ctrl') then
        ...
        write(MsgOut,'(A)') '  % ./crd_convert INPCNV'
        write(MsgOut,'(A)') ' '
        write(MsgOut,'(A)') '# usage to see control parameters'
        write(MsgOut,'(A)') '  % ./crd_convert -h ctrl'
        write(MsgOut,'(A)') ' '

      else

        write(MsgOut,'(A)') '# control parameters in crd_convert'
        write(MsgOut,'(A)') ' '

        call show_ctrl_input ('psf,ref,prmtop,ambcrd,grotop,grocrd')
        call show_ctrl_output('pdb,trj,rms,trr,STRJ')
      ...
      write(MsgOut,'(A)')'****************************************************'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*                     CRD_CONVERT                  *'
      write(MsgOut,'(A)')'*                                                  *'
      write(MsgOut,'(A)')'*           Converter of trajectory files          *'
      write(MsgOut,'(A)')'*                                                  *'
